...
import { ICreateOrEditCategoryDialogProps, TCreateOrEditCategoryFormValidation } from "./index.type";
import toast from "react-hot-toast";
import { useForm } from "react-hook-form";
import { UPSERT_CATEGORY_SUBJECT, createOrEditCategoryFormValidation } from "./index.constant";
import { zodResolver } from "@hookform/resolvers/zod";
import { createNewCategoryAction, editCategoryAction } from "@/app/(admin)/admin/categories/actions";
import Modal from "@/components/Modal";
import { Box, Button, CircularProgress, TextField } from "@mui/material";

const CreateOrEditCategoryDialog: FC<ICreateOrEditCategoryDialogProps> = ({ onClose, selectedCategory }) => {
  const [pending, handleTransition] = useTransition();
  const { register, handleSubmit, formState, setValue } = useForm<TCreateOrEditCategoryFormValidation>({
  ...
  const getFormError = (name: keyof TCreateOrEditCategoryFormValidation) => formState.errors?.[name]?.message;
      ...
      <form onSubmit={onCreateOrEditCategory}>
        ...
        <Box mt={4}>
          <Button type="submit" size="large" fullWidth>
            {pending ? <CircularProgress size="1.5rem" color="inherit" /> : <>Submit</>}
          </Button>
        </Box>
      </form>
    </Modal>
