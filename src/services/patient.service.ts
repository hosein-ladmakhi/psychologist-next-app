...
export const getPatientsPageApi = (data: Object) => {
  return httpGet<TPatientPageRes>(
    `${API_URL}/patient${prepareQueryParams(data)}`,
    undefined,
    ['patients'],
  );
};
...
export const createPatient = (data: TCreateOrEditPatientBody) =>
  httpPost<TCreateOrEditPatientBody, IPatient>(`${API_URL}/patient`, data);

export const editPatient = (id: number, data: TCreateOrEditPatientBody) =>
  httpPatch<TCreateOrEditPatientBody, IPatient>(
    `${API_URL}/patient/${id}`,
    data,
  );
